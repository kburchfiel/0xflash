...
private:
  PackedStringArray notification_array{}; // Will store
  // a limited set of recent updates for display within
  // the notification window

  int notification_message_limit = 50; // It may not
  // be necessary to limit the number of notifications shown,
  // but just in case displaying too many would
  // slow down the game, I'll use this value to
  // limit them.
  ...
  Array results_array{};
  int within_round_test_number =
      1; // Will get incremented by 1 after each test.

  // Creating a new ref-counted RandomNumberGenerator object:
  // (Based on
  // https://docs.godotengine.org/en/stable/engine_details/architecture/object_class.html#refcounted-memory-management
  // )
...
hexadecimal number ";
  ...
  const TypedDictionary<int, godot::String> id_game_mode_map{
      {0, "B16 to B10"}, {1, "B10 to B16"}, {2, "Multiplication"}};
...
public:
  ...
  // Based on:
  // https://docs.godotengine.org/en/stable/classes/class_lineedit.html#class-lineedit-signal-text-changed
  ...
  void update_notification_window(const godot::String new_notification);
